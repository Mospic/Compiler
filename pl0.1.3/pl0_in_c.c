...
void orderAdjust(int base)
{
	if(ox == 0)
		return ;
	else
	{
		instruction code_temp = code[base];
		for(int i = 0; i < ox; i++)
		{
			//memset(code[i], 0, sizeof(instruction));
			code[i + base] = code[i + base + 1];
		}
		//memset(code[ox], 0, sizeof(instruction));
		code[ox + base] = code_temp;
	}
}
